...
function start_airgeddon_from_tmux() {
  ...
  tmux send-keys -t "${session_name}:${tmux_main_window}" "clear;cd ${scriptfolder};bash ${scriptname} ${scan_d5_if} ${d2_if}" ENTER
  sleep 0.2
  if [ "${1}" = "normal" ]; then
...
function run_airodump() {
  ...
  tmux send-keys -t ${session_name}:0.1 "$airodump_command" Enter
  sleep 15
  # Kill airodump-ng
  tmux send-keys -t ${session_name}:0.1 C-c
}
